(() => {
  ...
  function refreshCupidRig() {
    ...
    // Responsive layout: bow stays lower-left and the arrow always points
    // toward the heart. Mobile gets a slightly tighter composition.
    const mobile = W <= 700;
    const gripX = mobile ? W * 0.24 : W * 0.23;
    const gripY = mobile ? H * 0.70 : H * 0.73;
    const heartX = mobile ? W * 0.69 : W * 0.68;
    const heartY = mobile ? H * 0.31 : H * 0.32;
    ...
    // SVG viewBox is 320x300. The nocking point is x=150,y=150.
    const scaleX = bR.width / 320;
    const scaleY = bR.height / 300;
    svgScale = scaleY || 1;

    const gripLX = (bR.left - aR.left) + 150 * scaleX;
    const gripLY = (bR.top - aR.top) + 150 * scaleY;
    ...
    arrowBaseY = nockLY - ((rR.top - aR.top) + 0.5 * rR.height);
    ...
    cupidMaxDraw = Math.min(bR.height * 0.70, H * (mobile ? 0.15 : 0.16), mobile ? 82 : 118);
    cupidCurDraw = 0;
    nockProxy.val = 150;
    applyNock();
  }
  ...
  async function fireCupid() {
    ...
    // Move the arrow along the exact vector from its tip to the blue heart.
    const dx = heartX - tipX;
    const dy = heartY - tipY;
    const distance = Math.hypot(dx, dy);
    const duration = Math.max(520, Math.min(900, distance * 1.25));
    const startX = arrowBaseX;
    const startY = arrowBaseY + cupidCurDraw;

    // Arrow already inherits the bow's aim angle. We only translate it.
    const endX = startX + dx / svgScale;
    const endY = startY + dy / svgScale;
    ...
    function fly(now) {
      ...
      arrow.style.transform =
        `translate(${startX + (endX - startX) * ease}px, ${startY + (endY - startY) * ease}px)`;
      arrow.style.opacity = "1";
