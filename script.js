(() => {
  ...
  async function playSurpriseVideo(){
    ...
    try{
      ...
      await new Promise((resolve,reject)=>{
        if(surpriseVideo.readyState>=3){resolve();return}
        const ok=()=>{cleanup();resolve()};
        const bad=()=>{cleanup();reject(new Error('video-load-failed'))};
        const cleanup=()=>{surpriseVideo.removeEventListener('canplay',ok);surpriseVideo.removeEventListener('error',bad)};
        surpriseVideo.addEventListener('canplay',ok,{once:true});
        surpriseVideo.addEventListener('error',bad,{once:true});
        setTimeout(()=>{cleanup();reject(new Error('video-timeout'))},8000);
      });
  ...
  surpriseVideo.addEventListener('error',()=>{surprisePlayButton.classList.add('show');surpriseVideoError.hidden=false});
  surpriseVideo.addEventListener('play',()=>surprisePlayButton.classList.remove('show'));
