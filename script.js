(() => {
  ...
  const memoriesButton=$('memoriesButton'), memoriesModal=$('memoriesModal'), closeMemories=$('closeMemories'), memoryNowPlaying=$('memoryNowPlaying'), memoryAtmosphere=$('memoryAtmosphere');
  ...
  function setMemoryAtmosphere(photoNumber){
    if(!memoryAtmosphere)return;
    const panel=memoryAtmosphere.closest('.memory-panel');
    if(!panel)return;
    panel.classList.remove('theme-1','theme-2','theme-3','theme-4','theme-5','theme-6');
    panel.classList.add(`theme-${photoNumber}`);
    memoryAtmosphere.classList.add('active');
  }

  function clearMemoryAtmosphere(){
    if(!memoryAtmosphere)return;
    const panel=memoryAtmosphere.closest('.memory-panel');
    if(panel)panel.classList.remove('theme-1','theme-2','theme-3','theme-4','theme-5','theme-6');
    memoryAtmosphere.classList.remove('active');
  }
  ...
  async function playMemorySong(photoNumber, figure){
    ...
    setMemoryAtmosphere(photoNumber);
    memorySong.src=song;
